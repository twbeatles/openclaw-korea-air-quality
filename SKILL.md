...
실행 가능한 CLI다. 현재 버전은 Open-Meteo 기반으로 한국 지역 대기질 조회가 가능하며, 지역명 해석, 기본 지역 저장, 여러 지역 비교, 알림 점검, 아침 브리핑, OpenClaw cron 초안 생성을 지원한다. `--provider airkorea` 레이어도 준비돼 있어 국내 API 연결 시 이 파일을 중심으로 확장한다.
